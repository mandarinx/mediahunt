...
<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover footable toggle-medium" data-filter="#filter">
        ...
        <tr>
            ...
            <td><a href="{{ url('user/'.$blog->user->username) }}">{{{ $blog->user->fullname }}}</a></td>
            <td><abbr class="timeago" title="{{ date(DATE_ISO8601,strtotime($blog->created_at)) }}">{{ date(DATE_ISO8601,strtotime($blog->created_at)) }}</abbr></td>
            <td>
                <a class="btn btn-success" href="{{ url('blog/'.$blog->id.'/'.$blog->slug) }}">
